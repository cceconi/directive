...
describe('ConfigSourceTracker', function (): void {
    ...
    it('returns null for an untracked key', function (): void {
        expect(ConfigSourceTracker::getSource('UNKNOWN_VAR_DIRECTIVE_TEST'))->toBeNull();
    });

    it('snapshotSystemVars() marks existing $_ENV keys as "system"', function (): void {
        $_ENV['DIRECTIVE_TRACKER_SYS'] = 'value';

        ConfigSourceTracker::snapshotSystemVars();

        expect(ConfigSourceTracker::getSource('DIRECTIVE_TRACKER_SYS'))->toBe('system');

        unset($_ENV['DIRECTIVE_TRACKER_SYS']);
    });
    ...
    it('snapshotSystemVars() does not overwrite already-tracked keys', function (): void {
        ...
        expect(ConfigSourceTracker::getSource('DIRECTIVE_TRACKER_PRE'))->toBe('.env');
    ...
    it('loadTracked() tracks variables from .env as ".env"', function (): void {
        ...
        expect(ConfigSourceTracker::getSource('DIRECTIVE_FROM_ENV'))->toBe('.env');

        @unlink($tmpDir . '/.env');
        @rmdir($tmpDir);
        unset($_ENV['DIRECTIVE_FROM_ENV']);
    });
    ...
    it('loadTracked() tracks variables from .env.local as ".env.local"', function (): void {
        ...
        expect(ConfigSourceTracker::getSource('DIRECTIVE_FROM_LOCAL'))->toBe('.env.local');

        @unlink($tmpDir . '/.env.local');
        @rmdir($tmpDir);
        unset($_ENV['DIRECTIVE_FROM_LOCAL']);
    });
    ...
    it('reset() clears all tracked entries', function (): void {
        $_ENV['DIRECTIVE_RESET_TEST'] = 'yes';
        ConfigSourceTracker::snapshotSystemVars();

        expect(ConfigSourceTracker::getSource('DIRECTIVE_RESET_TEST'))->toBe('system');

        ConfigSourceTracker::reset();

        expect(ConfigSourceTracker::getSource('DIRECTIVE_RESET_TEST'))->toBeNull();

        unset($_ENV['DIRECTIVE_RESET_TEST']);
    });
});
