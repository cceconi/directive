...
#[AsCommand(
    name: 'generate:usecase-query',
    description: 'Scaffold a UseCase with a Query input (UseCase, Query, Payload, Result, UseCaseInterface).',
)]
final class GenerateUseCaseQueryCommand extends DirectiveCommand
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addArgument('QueryName', InputArgument::OPTIONAL, 'Query name (e.g. GetOrderById)');
    }
    ...
    protected function executeCommand(InputInterface $input, OutputInterface $output): int
    {
        ...
        $name     = $this->resolveArg($input, $output, $helper, 'QueryName', 'Query name (e.g. GetOrderById)');

        if ($business === '' || $name === '') {
            $output->writeln('<error>Business and QueryName are required.</error>');
        ...
        $this->generator->generate($business, $name, $useCaseDir,   $namespace, ClassType::USE_CASE->value, ['input' => 'Query']);
        $this->generator->generate($business, $name, $useCaseDir,   $namespace, ClassType::QUERY->value);
        ...
        $output->writeln(sprintf('<info>✓</info> Scaffolded <comment>%s</comment> UseCase (query) in <comment>%s</comment>', $name, $useCaseDir));
